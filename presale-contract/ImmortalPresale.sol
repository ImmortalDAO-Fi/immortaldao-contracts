...
contract ImmortalPresale is Ownable {
    ...
    bool public whitelistedEnabled = true;
    ...
    function whitelistBuyers(address[] memory _buyers)
        ...
        returns (bool)
    {
        require(saleStarted() == false, "IDO already started");
    ...
    function purchaseIMMO(uint256 _amount) external returns (bool) {
        ...
        require(block.timestamp < endOfSale);
    ...
    function claim(address _recipient, bool _stake) public {
        ...
        uint256 amountRedeemed = amountAbleToRedeem.sub(
            amountClaimed[_recipient]
        );
        stakeOrSend(_recipient, _stake, amountRedeemed * decimal_IMMO);
        amountClaimed[_recipient] = amountClaimed[_recipient].add(
            amountRedeemed
        );
    }
   ...
   //50% unlocked at launch, other 50% linearly vested
    ...
    function withdraw() external onlyOwner {
        uint256 mcUSDInTreasury = 10000 * decimal_mcUSD; //10,000 mcUSD for initial IMMO backing

        IERC20(mcUSD).approve(treasury, mcUSDInTreasury);
        IMMOMinted = ITreasury(treasury).deposit(
            mcUSDInTreasury,
            mcUSD,
            (10000 - totalIMMObought) * decimal_IMMO
        );
    ...
    function enablePublic() external onlyOwner {
