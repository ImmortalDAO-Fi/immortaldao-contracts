...
contract ImmortalStaking is Ownable {
  ...
  constructor(
    ...
    uint256 _initialPercentageBurnt,
    uint256 _percentageReducedPerEpoch
  ) {
    ...
    percentageReducedPerEpoch = _percentageReducedPerEpoch;
  }
  ...
  function stake(uint256 _amount, address _recipient) external returns (bool) {
    ...
    IMMO_staked[_recipient] = IMMO_staked[_recipient].add(_amount);
    totalValue[_recipient] = totalValue[_recipient].add(
      _amount.mul(epoch.number)
    );
  ...
  function unstake(bool _trigger) external {
    ...
    uint256 avgEpoch = totalValue[msg.sender].div(IMMO_staked[msg.sender]);
    uint256 percentageReduced = avgEpoch.mul(percentageReducedPerEpoch);
